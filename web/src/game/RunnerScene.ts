...
const configs: Record<Difficulty, DifficultyConf> = {
  ...
  Medium: { speed: 330, levelSeconds: 40, patterns: ["single", "double", "triple", "platform"], gap: [190, 230] },
  Hard: {
    speed: 360,
    levelSeconds: 38,
...
type Obstacle = (Phaser.GameObjects.Rectangle | Phaser.GameObjects.Triangle) & { kind: "spike" | "platform" };
...
export class RunnerScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.player = this.add.rectangle(this.startX, this.GROUND_Y, 44, 44, 0xe9f0ff, 1).setStrokeStyle(3, 0x101018).setOrigin(0.5, 1);
    ...
    // Apply any pending difficulty change now that player exists.
  ...
  spawnPattern(startX: number) {
    ...
    const addSpike = (width = 28, height = 56) => {
      // Triangle with base on the ground line (y = GROUND_Y)
    ...
    const addPlatform = (width = 140, height = 14, elevation = 80) => {
      const ob = this.add.rectangle(x, this.GROUND_Y - elevation, width, height, 0x7890c0, 1) as Obstacle;
      ob.kind = "platform";
      ob.setOrigin(0, 1);
      this.obstacles.push(ob);

      // For tougher modes, block the ground beneath the platform so the player must use it.
      if (this.difficultyLevel !== "Easy") {
        const stripCount = Math.max(2, Math.floor(width / 26));
        const gapIndex = Phaser.Math.Between(1, stripCount - 2);
        for (let i = 0; i < stripCount; i++) {
          if (i === gapIndex) continue;
          const spike = this.add.rectangle(x + i * 26 + 12, this.GROUND_Y, 22, 44, color, 1) as Obstacle;
          spike.kind = "spike";
  ...
  update(time: number, delta: number) {
    ...
    for (const ob of this.obstacles) {
      if (ob.kind === "platform") {
        const orect = new Phaser.Geom.Rectangle(ob.getBounds().x, ob.getBounds().y, ob.displayWidth, ob.displayHeight);
      ...
      // Spike collision uses true triangle shape
      ...
      if (spikeHit) {
        this.dead = true;
        const over = this.sound.get("over");
        if (over) over.play({ volume: 0.8 });
  ...
  playStartSfx() {
    ...
    const start = this.sound.get("start");
    if (start) {
      start.play({ volume: 0.8 });
      this.startSfxAt = now;
  ...
  ensureBgm(retriesRemaining = 3) {
    ...
    if (retriesRemaining > 0) {
      this.time.delayedCall(200, () => this.ensureBgm(retriesRemaining - 1));
    } else {
      window.dispatchEvent(new CustomEvent("input-fallback", { detail: "Audio load failed for bgm" }));
    }
  }
